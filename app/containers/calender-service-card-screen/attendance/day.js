...
class Month extends React.Component {
  ...
  increaseQuantity = () => {
    const { item } = this.props;
    this.setState({
      quantity: item.service_type.id == CALENDAR_SERVICE_TYPES.MILK ? this.state.quantity + 0.25 : this.state.quantity + 1,
    }, () => {
      this.changeQuantity()
    });
  }
      ...
      <View style={styles.container}>
        ...
        {isPresent &&
          <View style={{ flexDirection: "row", width: 80 }}>
            ...
            <TouchableOpacity onPress={this.showEditQuantityModal} style={{ flex: 1, flexDirection: "row", justifyContent: "center", minWidth: 20 }}><Text>{quantity}</Text></TouchableOpacity>
